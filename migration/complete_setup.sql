...
-- without errors. It will create or update all necessary database objects.
--
...
-- Enable required PostgreSQL extensions (idempotent by default)
CREATE EXTENSION IF NOT EXISTS vector;
CREATE EXTENSION IF NOT EXISTS pgcrypto;
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- =====================================================
-- SECTION 2: DROP EXISTING FUNCTIONS AND TRIGGERS
-- =====================================================

-- Drop all triggers that depend on functions we might recreate
...
DROP FUNCTION IF EXISTS match_archon_crawled_pages(vector(1536), int, jsonb, text);
DROP FUNCTION IF EXISTS match_archon_code_examples(vector(1536), int, jsonb, text);
DROP FUNCTION IF EXISTS hybrid_search_archon_crawled_pages(vector(1536), text, int, jsonb, text);
DROP FUNCTION IF EXISTS hybrid_search_archon_code_examples(vector(1536), text, int, jsonb, text);
DROP FUNCTION IF EXISTS archive_task(uuid, text);

-- =====================================================
-- SECTION 3: CREDENTIALS AND SETTINGS TABLE
-- =====================================================
...
-- Add the generated column after table creation if it doesn't exist
DO $add_content_search_vector$ 
BEGIN
    IF NOT EXISTS (
    ...
    END IF;
END $add_content_search_vector$;

-- Add foreign key constraint (using DROP/ADD for idempotency)
ALTER TABLE archon_crawled_pages DROP CONSTRAINT IF EXISTS archon_crawled_pages_source_id_fkey;
ALTER TABLE archon_crawled_pages ADD CONSTRAINT archon_crawled_pages_source_id_fkey 
    FOREIGN KEY (source_id) REFERENCES archon_sources(source_id);

-- Create indexes for better performance (idempotent)
CREATE INDEX IF NOT EXISTS archon_crawled_pages_embedding_idx ON archon_crawled_pages USING ivfflat (embedding vector_cosine_ops);
...
BEGIN
    ...
    END IF;
END $add_code_search_vector$;

-- Add foreign key constraint (using DROP/ADD for idempotency)
ALTER TABLE archon_code_examples DROP CONSTRAINT IF EXISTS archon_code_examples_source_id_fkey;
ALTER TABLE archon_code_examples ADD CONSTRAINT archon_code_examples_source_id_fkey 
    FOREIGN KEY (source_id) REFERENCES archon_sources(source_id);

-- Create indexes for better performance (idempotent)
CREATE INDEX IF NOT EXISTS archon_code_examples_embedding_idx ON archon_code_examples USING ivfflat (embedding vector_cosine_ops);
...
-- =====================================================
-- SECTION 5: PROJECTS AND TASKS MODULE
-- =====================================================

-- Task status enumeration
-- Create task_status enum if it doesn't exist
DO $task_status_enum$ BEGIN
...
-- Add foreign key constraints (using DROP/ADD for idempotency)
ALTER TABLE archon_tasks DROP CONSTRAINT IF EXISTS archon_tasks_project_id_fkey;
ALTER TABLE archon_tasks ADD CONSTRAINT archon_tasks_project_id_fkey
    FOREIGN KEY (project_id) REFERENCES archon_projects(id) ON DELETE CASCADE;

ALTER TABLE archon_tasks DROP CONSTRAINT IF EXISTS archon_tasks_parent_task_id_fkey;
ALTER TABLE archon_tasks ADD CONSTRAINT archon_tasks_parent_task_id_fkey
...
-- Add foreign key constraint (using DROP/ADD for idempotency)
ALTER TABLE archon_project_sources DROP CONSTRAINT IF EXISTS archon_project_sources_project_id_fkey;
ALTER TABLE archon_project_sources ADD CONSTRAINT archon_project_sources_project_id_fkey
    FOREIGN KEY (project_id) REFERENCES archon_projects(id) ON DELETE CASCADE;
...
CREATE TABLE IF NOT EXISTS archon_document_versions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID,
  task_id UUID, -- DEPRECATED: No longer used, kept for historical data
...
-- Add foreign key constraints (using DROP/ADD for idempotency)
ALTER TABLE archon_document_versions DROP CONSTRAINT IF EXISTS archon_document_versions_project_id_fkey;
ALTER TABLE archon_document_versions ADD CONSTRAINT archon_document_versions_project_id_fkey
    FOREIGN KEY (project_id) REFERENCES archon_projects(id) ON DELETE CASCADE;

ALTER TABLE archon_document_versions DROP CONSTRAINT IF EXISTS archon_document_versions_task_id_fkey;
ALTER TABLE archon_document_versions ADD CONSTRAINT archon_document_versions_task_id_fkey
    FOREIGN KEY (task_id) REFERENCES archon_tasks(id) ON DELETE CASCADE;
...
-- =====================================================
-- SECTION 7: CREATE FUNCTIONS (AFTER ALL TABLES EXIST)
-- =====================================================
...
-- Create hybrid search functions ONLY after ensuring columns exist
DO $create_hybrid_functions$
BEGIN
    -- Only create hybrid search functions if the generated columns exist
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'archon_crawled_pages' 
        AND column_name = 'content_search_vector'
        ...
        LANGUAGE plpgsql
        AS $hybrid_search_crawled_pages$
        DECLARE
        ...
        LANGUAGE plpgsql
        AS $hybrid_search_code_examples$
        DECLARE
            max_vector_results INT;
            max_text_results INT;
        BEGIN
...
RETURNS BOOLEAN AS $archive_task_function$
DECLARE
    task_exists BOOLEAN;
BEGIN
    ...
    RETURN TRUE;
END;
$archive_task_function$ LANGUAGE plpgsql;
...
-- =====================================================
-- SECTION 8: CREATE TRIGGERS (AFTER FUNCTIONS EXIST)
-- =====================================================

-- Create triggers (now that function exists)
...
-- Drop ALL existing RLS policies before recreating them
...
DROP POLICY IF EXISTS "Allow authenticated users to read and update" ON archon_crawled_pages;
DROP POLICY IF EXISTS "Allow authenticated users to read and update" ON archon_sources;
DROP POLICY IF EXISTS "Allow authenticated users to read and update" ON archon_code_examples;
...
-- Knowledge base policies - Service role access
CREATE POLICY "Allow service role full access" ON archon_crawled_pages
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access" ON archon_sources
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access" ON archon_code_examples
    FOR ALL USING (auth.role() = 'service_role');

-- Knowledge base policies - Authenticated users
CREATE POLICY "Allow authenticated users to read and update" ON archon_crawled_pages
    FOR ALL TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read and update" ON archon_sources
    FOR ALL TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read and update" ON archon_code_examples
    FOR ALL TO authenticated
    USING (true);
...
-- Projects module policies - Service role access
...
-- Projects module policies - Authenticated users
...
-- =====================================================
-- SECTION 10: INITIAL SETTINGS DATA
-- =====================================================
...
--
-- This idempotent script can be run multiple times safely.
-- Next steps:
