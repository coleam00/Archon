...
function printHelp(): void {
  console.log(`HarnessLab version helper

Usage:
  bun scripts/harnesslab-version.ts current
  bun scripts/harnesslab-version.ts next [major|minor|patch]
  bun scripts/harnesslab-version.ts bump [major|minor|patch]
  bun scripts/harnesslab-version.ts set <version>
  bun scripts/harnesslab-version.ts tag [version]

Notes:
  - HarnessLab uses an independent SemVer line starting at 0.1.0.
...
if (command === 'current') {
  ...
  if (!value) {
    fail('Missing version. Example: bun scripts/harnesslab-version.ts set 0.1.0');
  }
