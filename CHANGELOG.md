...
- **A loop no longer has to invent a completion signal it doesn't use.** `until:` and `until_bash:` are now *at least one of* rather than `until:` unconditionally, so a loop whose completion is deterministic declares only the check — and then has no prose-matching path at all, which is the point: a sentinel the model happens to emit while reasoning about its own exit criteria can no longer end the loop early. Every existing workflow still loads and terminates unchanged. One behavior change comes with it: a `loop:` node now skips `until_bash` on an iteration whose signal already completed it, matching what `loop_group:` has always done. The two channels are OR'd, so the outcome is identical either way — only a side-effecting check runs one time fewer. (#2563)
