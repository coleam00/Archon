...
- `assistants.<provider>` in `.archon/config.yaml` is now validated when the config loads, by the same provider-owned parser a `--config` run layer already used. A setting the provider would have dropped — `modelReasoningEffort: extreme`, a misspelled key, a `settingSources` entry that is neither `project` nor `user` — stops the load with a message naming the file, the provider, the key, and the accepted values, instead of reaching a run and being reported in `node_started` as the depth the node ran at. `archon doctor` reports it as a failed **Config files** check, and the settings API refuses to write a value the loaders would then reject, answering `400` with the refused key (shown in the console's AI settings) rather than a generic `500`. A settings change that fixes an invalid value already in `~/.archon/config.yaml` is still saved; any other change is refused until that value is fixed. Valid configs are unaffected, and an entry for a provider this install has not registered is still ignored rather than rejected. Two OpenCode settings `config.yaml` used to accept now stop the load: `assistants.opencode.agent`, which nothing has read since it was introduced, and `assistants.opencode.baseUrl`, which every OpenCode run already rejected because Archon owns the embedded OpenCode runtime. (#2582)
...
- A worktree whose setup fails after `git worktree add` — git identity, submodule init, or configured file copies — is now removed instead of being adopted as ready by a later run. Archon creates every worktree locked (`git worktree add --lock`) and unlocks it only once its setup finishes, so a run that finds a locked one refuses to start on it and names the command that clears it, rather than working in a checkout with uninitialized submodules. (#3448)
