...
- A worktree whose setup fails after `git worktree add` — git identity, submodule init, or configured file copies — is now removed instead of being adopted as ready by a later run. Archon keeps such a worktree locked (`git worktree lock`) until its setup finishes, so a run that finds one refuses to start on it and names the command that clears it, rather than working in a checkout with uninitialized submodules. (#3448)
