...
- Chat-tick workspace sync of `~/.archon/workspaces/<owner>/<repo>/source/` is now **non-destructive by default**. Previously each non-slash chat message ran `git fetch && git reset --hard origin/<default_branch>`, silently destroying local commits, uncommitted edits, and non-default branches. The new default mode is `fast-forward`: `git fetch` runs, the local branch is fast-forwarded only when strictly behind origin, and all other states (`ahead`, `diverged`, `dirty`) are preserved. Worktree creation still hard-resets managed clones — that path is unchanged. An SSE advisory surfaces unpushed local work, and the new `WorkspaceSyncResult.state` field (`in_sync` / `behind` / `ahead` / `diverged` / `dirty`) is reported back to the UI. Codebases also persist their detected default branch in a new `default_branch` column, so non-`main` repos (e.g. `master`, `develop`) sync against the right branch instead of unconditionally against `main` (#1536, closes #1516).
