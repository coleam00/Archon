...
class TestDocumentStorageProgressIntegration:
    ...
    async def test_batch_progress_reporting(self, mock_credentials, mock_create_embeddings,
        ...
        # New signature: (message, progress, batch_info) where batch_info is a dict
        batch_calls = [call for call in mock_progress_callback.call_history
                      if len(call[0]) >= 3 and call[0][2] is not None and "current_batch" in call[0][2]]

        assert len(batch_calls) >= 2  # Should have multiple batch progress updates

        # Verify batch structure - batch_info is 3rd positional arg (index 2)
        for call_args, call_kwargs in batch_calls:
            batch_info = call_args[2]
            assert "current_batch" in batch_info
            assert "total_batches" in batch_info
            assert "completed_batches" in batch_info
            assert batch_info["current_batch"] >= 1
            assert batch_info["total_batches"] >= 1
            assert batch_info["completed_batches"] >= 0
    ...
    async def test_progress_callback_signature(self, mock_credentials, mock_create_embeddings,
        ...
        # New signature: (message: str, progress: int, batch_info: dict = None)
        callback_calls = []

        async def validate_callback(message: str, progress: int, batch_info: dict = None):
        ...
        for call in callback_calls:
            ...
            assert isinstance(call['batch_info'], dict)

            # Check that batch info contains expected fields when present
            if 'current_batch' in call['batch_info']:
                assert isinstance(call['batch_info']['current_batch'], int)
                assert isinstance(call['batch_info']['total_batches'], int)
                assert call['batch_info']['current_batch'] >= 1
                assert call['batch_info']['total_batches'] >= 1
    ...
    async def test_error_handling_in_progress_reporting(self, mock_credentials, mock_create_embeddings,
        ...
        # New signature: (message: str, progress: int, batch_info: dict = None)
        async def failing_callback(message: str, progress: int, batch_info: dict = None):
