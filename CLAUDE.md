...
**Model Validation:**
- Workflows are validated at load time for provider _identity_ only — `provider:` (workflow-level and per-node) must be a registered provider id, otherwise the YAML is rejected with `Unknown provider '<id>'. Registered: claude, codex, pi, omp`.
...
- Credentials (API keys + subscriptions) are injected into runs/chat env at execution time (vault always active — `TOKEN_ENCRYPTION_KEY` overrides the auto-key on managed deploys). Subscription tokens refresh-on-read and re-save on rotation. Subscriptions are delivered to native Claude/Codex, Pi, and OMP: native Claude direct chat gets `CLAUDE_CODE_OAUTH_TOKEN`, Codex workflow runs get `CODEX_HOME/auth.json`, Pi workflow runs get a per-run `auth.json` via `ARCHON_PI_AUTH_PATH` (and env-only direct chat uses `ANTHROPIC_OAUTH_TOKEN`), and OMP's request-env runtime bridge accepts `ANTHROPIC_OAUTH_TOKEN` or `CLAUDE_CODE_OAUTH_TOKEN` before `ANTHROPIC_API_KEY` for anthropic models.
