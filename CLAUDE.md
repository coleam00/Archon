...
- The workflow YAML expresses only what the ENGINE must see to govern a run: ordering, gates, joins, retries, sessions, artifacts, reusable structure. Computation stays out of the YAML and lives inside nodes. This boundary is what keeps load-time validation, the visual builder, resume, and audit trails possible.
