...
- `POST /api/workflows/runs/{runId}/resume` - Mark a failed run as ready for resume; the next invocation with `--resume` (CLI) or via the Web UI will skip already-completed nodes
