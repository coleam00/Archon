---
description: 'Batch-mode red-team integration — parse a filled findings report and atomically apply each resolution to its forward-facing spec (or, with --allow-historical-edits, to historical SpecKit working records). Mirror of /speckit-clarifybatch --apply for red-team. Pair with /speckit.red-team.draft (which produces the findings file).'
# No MCP tools required — this command performs file edits via the host AI
# agent's built-in primitives (Read / Edit / Write).
---
