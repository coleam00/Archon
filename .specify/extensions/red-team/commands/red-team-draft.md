---
description: 'Run adversarial review of a functional spec and produce a structured findings report; STOP after the report is written (no interactive resolution walk). Use /speckit.red-team.apply to batch-integrate resolutions from the file. Mirrors /speckit.red-team.run §1-§6 verbatim — same args, same lens-dispatch behaviour.'
...
4. **If `len(matched_lenses) > 5`**: Enter the propose-and-confirm flow:
   ...
   - **Otherwise** (no `--yes`): prompt the maintainer to respond:
     ...
     _(CI / batch runs MUST pass `--yes` to auto-accept the proposed default; running without `--yes` in a non-interactive context will stall waiting for input. This keeps the behavior simple: interactivity is determined by whether `--yes` was passed, not by detecting the terminal.)_
