...
- **Test an upgrade, not just a fresh install.** `bun run check:schema-upgrades` applies the current schema on top of databases created by older release tags and asserts the result converges on the fresh-install schema. It needs a reachable PostgreSQL (`PGHOST`/`PGUSER`/… or `DATABASE_URL`; it creates and drops its own scratch databases), so it is a CI job rather than part of `bun run validate`.
