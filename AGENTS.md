...
- **Test an upgrade, not just a fresh install.** `bun run check:schema-upgrades` applies the current schema on top of databases created by older release tags, re-applies it to prove idempotence, and compares the result against a fresh install across columns, indexes, column comments, constraints and sequences. Known-and-sanctioned divergences live in a small `ALLOWED_DRIFT` allowlist in the script and are printed on every run, never silently skipped. Baselines are a sample — the five most recent tags plus the oldest one carrying the schema — not every released version. It needs a reachable PostgreSQL (`PGHOST`/`PGUSER`/… or `DATABASE_URL`; it creates and drops its own scratch databases), so it is a CI job rather than part of `bun run validate`.
