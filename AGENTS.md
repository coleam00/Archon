...
Either way prefer removing work over widening the window: #2310 dropped a `tar` spawn for an in-process fixture, #2306 stopped a dry run creating a database, and #2558 replaced per-entry `sed`/`basename`/`dirname` forks with shell builtins. Best of all, keep subprocess-spawning tests rare and cheap in the first place — every one of them is a test whose cost you do not control.
