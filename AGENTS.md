...
- `$ADAPTER` / `$CHANNEL_ID` / `$CHANNEL_NAME` - Which adapter and channel triggered this run (from the message's `ChannelReference`, defined in `@archon/workflows/deps` and re-exported from `@archon/core/types`). `channelId` is NOT always the same as the run's conversation — see `packages/docs-web/src/content/docs/reference/variables.md`. Empty string when absent (fail-open, like `$DOCS_DIR`) for prompt-text substitution; delivered to `bash:`/`script:` nodes as env vars (`ADAPTER`/`CHANNEL_ID`/`CHANNEL_NAME`, no `ARCHON_` prefix) rather than spliced into shell text, since channel/adapter values originate from external platform data — but when no `ChannelReference` exists, those three keys are OMITTED from the subprocess env entirely (not set to `''`), so an unset channel never clobbers a same-named configured project env var.
