...
│   └── src/
│       ├── clients/          # AI SDK clients (Codex, Codex)
...
8. **`codebase_env_vars`** - Per-project env vars injected into Codex SDK subprocess env (managed via Web UI or `env:` in config)
...
- **ClaudeClient**: `@anthropic-ai/Codex-agent-sdk`
...
assistants:
  Codex:
    ...
    settingSources: # Controls which AGENTS.md files Codex SDK loads
...
- Codex models: `sonnet`, `opus`, `haiku`, `Codex-*`, `inherit`
- Codex models: Any model except Codex-specific aliases
