...
### `provider` (optional)
- **Type**: `'codex'`
- **Default**: falls back to `.archon/config.yaml` assistants default (Codex)
...
| `allowed_tools` | string[] | all tools | `[]` = no built-in tools |
| `denied_tools` | string[] | none | Removes named tools from default set |
...
| `output_format` | object | none | JSON Schema for structured output |
| `allowed_tools` | string[] | all | Tool whitelist. `[]` = no tools |
| `denied_tools` | string[] | none | Tool blacklist |
