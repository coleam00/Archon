...
provider: codex                     # 'codex' (default: from config)
...
| `provider` | `codex` | inherited | Per-node provider override |
...
| `allowed_tools` | string[] | all | Tool whitelist. `[]` = disable all |
| `denied_tools` | string[] | none | Tool blacklist |
| `retry` | object | 2 retries, 3s | Retry config. **Hard error on loop nodes** |
| `hooks` | object | — | SDK hooks. See `dag-advanced.md` |
| `mcp` | string | — | MCP config path. See `dag-advanced.md` |
| `skills` | string[] | — | Skill names. See `dag-advanced.md` |
