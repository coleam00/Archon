...
- When the requested scope is a recorded pull request — delivery passes its verified record as JSON, carrying `repo` and `number` — review exactly that pull request. Use its qualified `repo` and `number` for every forge read; do not resolve a PR from the current branch, and do not accept a different target. The review object is that PR's diff, exactly.
