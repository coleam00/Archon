...
- When the requested scope is a recorded pull request — delivery passes its verified record as JSON, carrying `repo`, `number`, `head` and `base` — review exactly that pull request. The record already names the target, so read nothing from a forge to establish it: the review object is the diff of that head against that base, which this checkout already has (`git diff <base>...HEAD`). Do not resolve a PR from the current branch, and do not accept a different target.
