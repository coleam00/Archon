...
#
# Written to $ARTIFACTS_DIR, NOT /tmp. Archon runs workflows concurrently and
# worktree isolation does not isolate /tmp, so a fixed host-global path lets two
# runs clobber each other's allow-list — run A then measures its own edits
# against run B's list, sees them as out-of-scope, and reverts real work with no
# error. $ARTIFACTS_DIR is per-run, pre-created by the executor, and lives
# outside the worktree so it can never be staged.
git diff origin/$BASE_BRANCH...HEAD --name-only | sort -u > "$ARTIFACTS_DIR/scope-allowlist.txt"
echo "In-scope files (this is the ONLY set you may edit):"; cat "$ARTIFACTS_DIR/scope-allowlist.txt"
```
...
**Scope gate — run this BEFORE staging.** Every file you changed must be in the allow-list (1.3), or a new test/doc paired 1:1 with an in-scope source file. Anything else is a scope leak and must not be committed. Only stage files you actually changed — and never repo-local Archon telemetry (`.archon/artifacts/`, `.archon/logs/`, `.archon/state/` are local-only, never in git):

```bash
# Tracked modifications AND untracked new files. `git diff --name-only` lists only
# tracked changes, so a stray NEW out-of-scope file would never appear here — while
# the instruction below tells you to `rm` exactly such files. Union both so the gate
# actually catches what it claims to.
{ git diff --name-only; git ls-files --others --exclude-standard; } | sort -u > "$ARTIFACTS_DIR/changed.txt"
# Files you changed that are NOT in the allow-list:
comm -23 "$ARTIFACTS_DIR/changed.txt" "$ARTIFACTS_DIR/scope-allowlist.txt"
```

For each path that prints: if it is **not** a new test/doc paired 1:1 with an in-scope source file, **and not a mechanical consequence (see below)**, it is **out of scope** — revert it (`git checkout -- <path>` for an edit, or `rm <path>` for a stray new file) and move the finding to "Suggested Follow-up Issues." Re-run the check until it prints only justified paths (ideally nothing).

**Mechanical consequences are in scope.** A file that *must* change as a deterministic result of an in-scope edit is allowed — generated bundles (`bun run generate:bundled`, `generate:bundled-schema`, `check-bundled-skill`), lockfiles, and manifest entries such as a `package.json` test split. Without this carve-out the gate reverts the regeneration an in-scope change requires and hands back a red PR plus a pointless follow-up issue. Two conditions: produce it by running the **documented command**, never by hand-editing the generated file, and name it in the fix report under "Mechanical consequences" with the command you ran.
...
Files that changed only because an in-scope edit deterministically required it —
generated bundles, lockfiles, manifest entries. Each must name the command that
produced it, so a reviewer can reproduce it rather than trust it.
