...
Read `packages/core/src/clients/codex.ts` first 80 lines - `IAssistantClient` implementation,
thread setup, and streaming event loop pattern.
...
Read `packages/server/src/index.ts` first 80 lines - adapter wiring, auth gate behavior, and
Hono app setup.
...
### AI Clients
- `CodexClient` (codex-sdk) is the supported runtime path
...
### Server
- Port allocation and adapter wiring
- Web API + SSE paths
