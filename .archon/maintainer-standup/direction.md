...
- **Bundled defaults are reference implementations — composition is how we author them, not a contract we offer.** The bundled set demonstrates engine capabilities and workflow-authoring best practice, and stays stack-agnostic so it runs on any project (rewrite tracked in #2123: primitive-focused SDLC set composed via `include:`). Bundled `include:` blocks are internal structure plus modeled best practice (declare `returns:`), not a stable block library: contracts hold within one bundle version, never across releases. User copies are expected to diverge — building on a bundled block is allowed, neither mandatory nor recommended; the best workflows are project-specific. Repo-override by name is the pinning mechanism. Cite as `direction.md §defaults-reference-implementations`.
