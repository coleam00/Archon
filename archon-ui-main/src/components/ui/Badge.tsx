...
// Type definitions for safe indexed access
type BadgeColor =
  | "purple"
  | "green"
  | "pink"
  ...
  | "red"
  | "yellow";
type NonSemanticVariant = "solid" | "outline";
...
export const Badge: React.FC<BadgeProps> = ({
  ...
  if (
    ...
    const semanticClasses = {
      ...
      primary:
        "bg-blue-500/10 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 border border-blue-500/20",
      secondary:
  ...
  const effectiveColor: BadgeColor = (color ?? "gray") as BadgeColor;
  const narrowedVariant: NonSemanticVariant = variant as NonSemanticVariant;
  const colorMap = {
    solid: {
      purple:
        "bg-purple-500/10 text-purple-500 dark:bg-purple-500/10 dark:text-purple-500",
      green:
        "bg-emerald-500/10 text-emerald-500 dark:bg-emerald-500/10 dark:text-emerald-500",
      ...
      yellow:
        "bg-yellow-500/10 text-yellow-500 dark:bg-yellow-500/10 dark:text-yellow-500",
    },
    outline: {
      purple:
        "border border-purple-300 text-purple-600 dark:border-purple-500/30 dark:text-purple-500",
      green:
        "border border-emerald-300 text-emerald-600 dark:border-emerald-500/30 dark:text-emerald-500",
      ...
      orange:
        "border border-orange-500 text-orange-500 dark:border-orange-500 dark:text-orange-500 shadow-[0_0_10px_rgba(251,146,60,0.3)]",
      red: "border border-red-300 text-red-600 dark:border-red-500/30 dark:text-red-500",
      yellow:
        "border border-yellow-300 text-yellow-600 dark:border-yellow-500/30 dark:text-yellow-500",
    },
  };
  ...
  return (
    <span
      className={`
        inline-flex items-center rounded
        ${sizeClasses[size]}
        ${colorMap[narrowedVariant][effectiveColor]}
        ${className}
      `}
      {...props}
    >
