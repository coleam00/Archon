...
export const ArchonChatPanel: React.FC<ArchonChatPanelProps> = (props) => {
    ...
    try {
      ...
      try {
        ...
        try {
          await agentChatService.streamMessages(
            ...
            (error: Error) => {
              ...
              setConnectionError(
                "Chat service is offline. Messages will not be received."
              );
            }
          );
        } catch (error) {
        ...
        if (error instanceof Error && error.message.includes("not available")) {
          setConnectionError(
            "Agent chat service is disabled. Enable it in docker-compose to use this feature."
          );
        } else {
          setConnectionError(
            "Failed to initialize chat. Server may be offline."
          );
        }
        setConnectionStatus("offline");
      }
      ...
      if (error instanceof Error && error.message.includes("not available")) {
        ...
        setConnectionError(
          "Failed to connect to agent. Server may be offline."
        );
      }
      setConnectionStatus("offline");
    }
  ...
  return (
    ...
    >
      ...
      <div
        ref={dragHandleRef}
        className={`absolute left-0 top-0 w-1.5 h-full cursor-ew-resize z-20 ${
          isDragging ? "bg-blue-500/50" : "bg-transparent hover:bg-blue-500/30"
        ...
        <div className="p-4 border-t border-gray-200 dark:border-zinc-800/80 bg-white/60 dark:bg-transparent">
          {connectionStatus === "offline" && (
            <div className="mb-3 p-3 bg-red-50/80 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-md">
              <div className="flex items-center text-sm text-red-700 dark:text-red-300">
                ...
                Chat is currently offline. Please use the reconnect button above
                to try again.
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={
                connectionStatus !== "online" || isTyping || !inputValue.trim()
              }
            ...
            >
              ...
              {/* Inner glow effect */}
              <div
                className={`absolute inset-[1px] rounded-sm bg-blue-100/30 dark:bg-blue-500/10 ${
                  isTyping || connectionStatus !== "online"
                    ? "opacity-20"
              ...
              <Send
                className={`w-4 h-4 text-blue-500 dark:text-blue-400 relative z-10 ${
                  isTyping || connectionStatus !== "online"
                    ? "opacity-50"
