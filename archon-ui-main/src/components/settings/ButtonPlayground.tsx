...
export const ButtonPlayground: React.FC = () => {
      ...
      if (linked[corner]) {
        ...
        Object.keys(linked).forEach((key) => {
          if (linked[key as keyof CornerRadius]) {
            ...
            newRadius[key as keyof CornerRadius] =
              layer1Radius[key as keyof CornerRadius];
          }
        });
        setRadius(newRadius);
      } else {
      ...
      if (linked[corner]) {
        ...
        Object.keys(linked).forEach((key) => {
          if (linked[key as keyof CornerRadius]) {
            ...
            newRadius[key as keyof CornerRadius] =
              layer2Radius[key as keyof CornerRadius];
          }
        });
        setRadius(newRadius);
      } else {
  ...
  background: ${
    layer1Color === "none" ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.9)"
  };
  background: ${
    layer1Color === "none" ? "rgba(0,0,0,0.9)" : "rgba(0,0,0,0.9)"
  } !important; /* Dark mode */
    ...
    layer1Border
      ? `border: 1px solid ${
          layer1Color === "none"
            ...
            : getColorConfig(layer1Color).border.split(" ")[1]
        };`
      : ""
  }
  ${
    layer1Glow !== "none"
      ? `box-shadow: 0 0 ${getGlowConfig(layer1Glow).blur}px ${
          getColorConfig(layer1Color).glow
        };`
      : ""
  }
}
  ...
  ${
    layer2Border
      ? `border: 1px solid ${
          layer2Color === "none"
            ? "rgba(255,255,255,0.2)"
            : getColorConfig(layer2Color).border.split(" ")[1]
        };`
      : ""
  }
  ${
    layer2Glow !== "none"
      ? `box-shadow: 0 0 ${getGlowConfig(layer2Glow).blur}px ${
          getColorConfig(layer2Color).glow
        };`
      : ""
  }
  pointer-events: none;
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        value={layer1Color}
                        onChange={(e) =>
                          setLayer1Color(e.target.value as ColorOption)
                        }
                    ...
                    <div>
                      ...
                      <select
                        value={layer1Glow}
                        onChange={(e) =>
                          setLayer1Glow(e.target.value as GlowIntensity)
                        }
                        className="w-full px-2 py-1 text-sm bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        value={borderGlow}
                        onChange={(e) =>
                          setBorderGlow(e.target.value as GlowIntensity)
                        }
                        className="w-full px-2 py-1 text-sm bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded"
                      >
                        {glowOptions.map((option) => (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        value={layer2Color}
                        onChange={(e) =>
                          setLayer2Color(e.target.value as ColorOption)
                        }
                        className="w-full px-2 py-1 text-sm bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded"
                        disabled={!showLayer2}
                      >
                    ...
                    <div>
                      ...
                      <select
                        value={layer2Glow}
                        onChange={(e) =>
                          setLayer2Glow(e.target.value as GlowIntensity)
                        }
                        className="w-full px-2 py-1 text-sm bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded"
                        disabled={!showLayer2}
                      >
