...
export const CrawlingProgressCard: React.FC<CrawlingProgressCardProps> = ({
  ...
  const displayData = progressData
    ? {
        ...
        currentUrl:
          ...
          progressData.current_url ||
          initialData?.currentUrl,
        crawlType:
          progressData.crawlType ||
          progressData.crawl_type ||
          initialData?.crawlType,
      }
    : ({
        progressId,
        status: "starting",
  ...
  const logsContainerRef = useTerminalScroll(displayData?.logs || [], showLogs);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (progressData.status === "error" || progressData.status === "failed") &&
      !hasHandledError &&
      onError
    ) {
  ...
  const statusConfig = (() => {
    const config =
      STATUS_CONFIG[displayData.status as keyof typeof STATUS_CONFIG];
    if (config) {
      return config;
    }
    ...
    console.warn(
      `Unknown status: ${displayData.status}, progress: ${displayData.progress}%, message: ${displayData.message}`
    );
  ...
  useEffect(() => {
    if (
      displayData.status === "finalization" ||
  ...
  const isActive = !["completed", "error", "failed", "cancelled"].includes(
    displayData.status || ""
  );
  ...
  return (
    <Card className="relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-4">
        <Badge
          color={crawlType.label.includes("Sitemap") ? "purple" : "blue"}
          variant="solid"
        >
          {crawlType.icon}
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-gray-200 dark:bg-zinc-700 rounded-full h-2">
            <motion.div
              ...
              animate={{
                width: `${Math.max(
                  0,
                  Math.min(100, displayData.progress || 0)
                )}%`,
              }}
          ...
          {displayData.message && (
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {displayData.message}
              {displayData.status === "crawling" &&
                displayData.totalPages !== undefined &&
      ...
      {isActive &&
        displayData.status === "crawling" &&
        (displayData.totalPages > 0 || displayData.processedPages > 0) && (
      ...
      {isActive && displayData.status === "code_extraction" && (
        <div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20 rounded-md">
          ...
          {(displayData.completedDocuments !== undefined ||
            displayData.totalDocuments !== undefined) &&
            displayData.completedDocuments < displayData.totalDocuments && (
              <div className="mb-2">
                <div className="text-xs text-yellow-600 dark:text-yellow-400/80">
                  Scanning documents: {displayData.completedDocuments || 0} /{" "}
                  {displayData.totalDocuments || 0}
                </div>
                <div className="w-full bg-yellow-200/50 dark:bg-yellow-700/30 rounded-full h-1.5 mt-1">
                  <div
                    ...
                    style={{
                      width: `${Math.round(
                        ((displayData.completedDocuments || 0) /
                          Math.max(1, displayData.totalDocuments || 1)) *
                          100
                      )}%`,
                    }}
                  />
          ...
          {(displayData.completedSummaries !== undefined ||
            displayData.totalSummaries !== undefined) &&
            displayData.totalSummaries > 0 && (
              <div className="mb-2">
                <div className="text-xs text-yellow-600 dark:text-yellow-400/80">
                  Generating summaries: {displayData.completedSummaries || 0} /{" "}
                  {displayData.totalSummaries || 0}
                </div>
                <div className="w-full bg-yellow-200/50 dark:bg-yellow-700/30 rounded-full h-1.5 mt-1">
                  <div
                    ...
                    style={{
                      width: `${Math.round(
                        ((displayData.completedSummaries || 0) /
                          Math.max(1, displayData.totalSummaries || 1)) *
                          100
                      )}%`,
                    }}
                  />
          ...
          {!displayData.codeBlocksFound &&
            displayData.details?.codeBlocksFound !== undefined && (
              <div className="flex items-center gap-4">
                ...
                {displayData.details?.totalChunks && (
                  <div className="text-xs text-yellow-600 dark:text-yellow-400/60">
                    Scanning chunk {displayData.details.currentChunk || 0} of{" "}
                    {displayData.details.totalChunks}
                  </div>
                )}
              </div>
      ...
      {isActive && displayData.status === "document_storage" && (
        <div className="mb-4 grid grid-cols-2 gap-3">
          {displayData.details?.currentChunk !== undefined &&
            displayData.details?.totalChunks && (
              <div className="p-2 bg-blue-50 dark:bg-blue-500/10 rounded-md">
                ...
                <div className="text-lg font-bold text-blue-700 dark:text-blue-300">
                  {displayData.details.currentChunk} /{" "}
                  {displayData.details.totalChunks}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-400/80">
                  {Math.round(
          ...
          {displayData.details?.embeddingsCreated !== undefined && (
            <div className="p-2 bg-green-50 dark:bg-green-500/10 rounded-md">
              ...
              <div className="text-xs text-green-600 dark:text-green-400/80">
                created
              </div>
            </div>
          )}
          ...
          {displayData.details?.codeBlocksFound !== undefined &&
            displayData.status === "code_extraction" && (
          ...
          {displayData.details?.chunksPerSecond && (
            <div className="p-2 bg-purple-50 dark:bg-purple-500/10 rounded-md">
              ...
              <div className="text-xs text-purple-600 dark:text-purple-400/80">
                chunks/sec
              </div>
            </div>
          )}
          ...
          {displayData.details?.estimatedTimeRemaining && (
            <div className="p-2 bg-orange-50 dark:bg-orange-500/10 rounded-md">
              ...
              <div className="text-xs text-orange-600 dark:text-orange-400/80">
                estimated
              </div>
            </div>
          )}
      ...
      {(() => {
        const shouldShowBatch =
          displayData.totalBatches &&
          displayData.totalBatches > 0 &&
          isActive &&
          displayData.status === "document_storage";
        ...
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-bold text-blue-600 dark:text-blue-400">
              {displayData.completedBatches || 0}/{displayData.totalBatches}{" "}
              batches
            </span>
          </div>
          ...
          <div className="w-full bg-blue-200 dark:bg-blue-900/50 rounded-full h-1.5 mb-2">
            <motion.div
              ...
              animate={{
                width: `${Math.round(
                  ((displayData.completedBatches || 0) /
                    displayData.totalBatches) *
                    100
                )}%`,
              }}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {displayData.activeWorkers !== undefined && (
              <div className="text-blue-600 dark:text-blue-400/80">
                <span className="font-medium">{displayData.activeWorkers}</span>{" "}
                parallel{" "}
                {displayData.activeWorkers === 1 ? "worker" : "workers"}
              </div>
            )}
            ...
            {displayData.currentBatch && displayData.totalChunksInBatch && (
              <div className="text-blue-600 dark:text-blue-400/80">
                ...
                <span className="font-medium">
                  {displayData.chunksInBatch || 0}/
                  {displayData.totalChunksInBatch}
                </span>{" "}
                chunks
              </div>
            )}
            ...
            {displayData.details?.totalChunks && (
              <div className="text-blue-600 dark:text-blue-400/80 col-span-2">
                Total progress:{" "}
                <span className="font-medium">
                  {displayData.details.currentChunk || 0}/
                  {displayData.details.totalChunks}
                </span>{" "}
                chunks processed
              </div>
                  ...
                  <div className="flex-1">
                    ...
                    {step.status === "active" && (
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {step.id === "document_storage" &&
                        displayData.completedBatches !== undefined &&
                        displayData.totalBatches ? (
                          <span>
                            Batch {displayData.completedBatches + 1} of{" "}
                            {displayData.totalBatches}
                          </span>
                        ) : step.id === "code_extraction" &&
                          displayData.details?.codeBlocksFound !== undefined ? (
                          <span>
                            {displayData.details.codeBlocksFound} code blocks
                            found
                          </span>
                        ) : step.id === "crawling" &&
                          (displayData.processedPages !== undefined ||
                            displayData.totalPages !== undefined) ? (
                          <span>
                            {displayData.processedPages !== undefined
                              ? displayData.processedPages
                              : "?"}{" "}
                            of{" "}
                            {displayData.totalPages !== undefined
