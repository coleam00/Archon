...
import { LinkIcon, Upload, BoxIcon, Brain } from "lucide-react";
...
export const AddKnowledgeModal = ({
  ...
  const [knowledgeType, setKnowledgeType] = useState<"technical" | "business">(
    "technical"
  );
  ...
  const validateUrl = async (
    url: string
  ): Promise<{ isValid: boolean; error?: string; formattedUrl?: string }> => {
    try {
      ...
      if (tld.length < 2) {
        return {
          isValid: false,
          error: "Please enter a valid domain with a proper extension",
        };
      }
