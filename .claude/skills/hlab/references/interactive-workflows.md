...
Check `hlab workflow status` periodically. When status changes to `paused`, the workflow is waiting for user input.
...
```bash
hlab workflow approve <run-id> "<user's exact response>"
```
...
hlab workflow approve <run-id> "your feedback or answers here"

# Reject (cancels the workflow)
hlab workflow reject <run-id> "reason for rejection"
```
