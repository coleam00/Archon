...
**Important**: The server runs from the **hlab repo root** — not the target repo. The `.env` file should already exist there (created in Step 4 of the setup wizard).

## 1. Verify `.env`

Make sure `.env` exists in the hlab repo root with the platform tokens from the platform-specific guides:

```bash
ls -la <harneeslab-repo>/.env
```

If it doesn't exist yet:
```bash
cd <harneeslab-repo> && cp .env.example .env
```
...
```bash
cd <harneeslab-repo> && bun run dev
```
...
**Docker (production):**
```bash
cd <harneeslab-repo> && docker-compose --profile with-db up -d --build
```
