# HarneesLab Configuration Guide

Interactive guide for viewing and modifying HarneesLab configuration. Use this when the user wants to change, view, or understand their config — at any point, not just during initial setup.
...
Options:
  1. "Bot name" — Display name shown in messages (current: {value}, default: HarneesLab)
...
After writing, confirm: "Updated {path}. Changes take effect on the next HarneesLab invocation (no restart needed for CLI)."
...
|--------|------|---------|-------------|
| `botName` | string | `HarneesLab` | Bot display name shown in messages |
