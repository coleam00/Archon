...
The user is inside the **remote-coding-agent** repository — that's how they have access to this skill. The HarneesLab repo path is the current working directory. Store it as `<harneeslab-repo>`.

## Step 1: Ask for Target Repo

**IMPORTANT**: The target repo is the user's own project — **never** the remote-coding-agent (HarneesLab) repo itself. Do not suggest or offer the current directory as an option.
...
1. `cd <harneeslab-repo> && ~/.bun/bin/bun install` (or `bun install` if bun was already in PATH)
2. `cd <harneeslab-repo>/packages/cli && ~/.bun/bin/bun link` (or `bun link`)
...
**CRITICAL**: Do NOT run `hlab setup` directly via Bash — it requires interactive input that I cannot provide. The `--spawn` flag attempts to open a new terminal window where the user can interact directly.
...
Bundled default commands and workflows are loaded automatically at runtime from the HarneesLab installation — nothing needs to be copied into the target repo.
...
After all platform tokens are in `.env`, read and follow `guides/server.md` to start the server from the hlab repo.
...
```bash
cd <target-repo> && hlab workflow list
```
...
```
Header: "HarneesLab skill"
Question: "Would you like to copy the HarneesLab skill into your target repo so Claude Code can invoke HarneesLab workflows from there?"
Options:
  1. "Yes, copy the skill" — Copies .claude/skills/hlab/ into the target repo. This will appear in git unless you gitignore it.
  2. "No, skip" — You can always run HarneesLab workflows from the HarneesLab repo instead.
```
...
cp -r <harneeslab-repo>/.claude/skills/hlab <target-repo>/.claude/skills/hlab
```
...
4. The hlab skill is now loaded — ask Claude to run workflows, fix issues, review PRs, etc.

Example first command in the target repo:
```
"Use hlab to fix issue #1"
```
...
> "If you want to configure advanced options later — like changing the default AI assistant, customizing worktree behavior, or adjusting which files get copied into isolated environments — just ask me to help you with 'hlab config' and I'll walk you through it."

The end state: user is in their target repo with the HarneesLab skill available, defaults loaded at runtime without polluting the repo, using Claude Code as the interface.
