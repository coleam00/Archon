...
Steps to install and configure the HarneesLab CLI. Always run these first.

## 1. Install Dependencies

```bash
cd <harneeslab-repo> && bun install
```
...
```bash
cd <harneeslab-repo>/packages/cli && bun link
```

This makes `hlab` available as a global command from any directory.
