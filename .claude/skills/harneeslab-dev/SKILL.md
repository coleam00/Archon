...
name: harneeslab-dev
description: |
  The PRIMARY development workflow for the HarneesLab project (remote-coding-agent).
  Use this skill instead of any PRP skills when working on HarneesLab code.
  ...
  IMPLEMENT   — "implement the plan", "execute .claude/harneeslab/plans/auth.plan.md",
  ...
  NOT for: Running HarneesLab CLI workflows in worktrees (use /hlab instead).
...
- **Artifacts**: !`ls .claude/harneeslab/ 2>/dev/null || echo "none yet"`
- **Active plans**: !`ls .claude/harneeslab/plans/*.plan.md 2>/dev/null | head -5 || echo "none"`
...
All artifacts go to `.claude/harneeslab/`. Create subdirectories as needed on first use.
