...
- Files committed
- Suggest next step: `/harneeslab-dev pr` to create a pull request

### Examples

```
/harneeslab-dev commit                          # All changes
/harneeslab-dev commit typescript files         # *.ts only
/harneeslab-dev commit except package-lock      # Exclude specific
/harneeslab-dev commit only the new files       # Untracked only
/harneeslab-dev commit staged                   # Already-staged only
/harneeslab-dev commit the auth refactor        # Context-based targeting
```
