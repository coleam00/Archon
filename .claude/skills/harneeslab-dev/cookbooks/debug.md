...
**If from a GitHub issue**: Save to `.claude/harneeslab/issues/issue-{number}.md`
**Otherwise**: Save to `.claude/harneeslab/debug/{date}-{slug}.md`
...
**Next steps**:
- For complex fixes: `/harneeslab-dev plan` (create a plan from the RCA)
- For simple fixes: `/harneeslab-dev implement` (implement directly)
