...
- {For bugs}: `/harneeslab-dev debug #{number}` to investigate
- {For features}: `/harneeslab-dev prd` to write requirements, or `/harneeslab-dev plan` to design
```
...
```
/harneeslab-dev issue                              # Use recent conversation context
/harneeslab-dev report this to gh                  # Same — infer from context
/harneeslab-dev log this bug in github             # Bug from context
/harneeslab-dev create a feature request for X     # Feature with description
/harneeslab-dev gh issue: streaming breaks on large responses  # Bug with description
```
