...
2. **If no path**: Look for the most recent `.plan.md` in `.claude/harneeslab/plans/` (excluding `completed/`)
3. **If no plans found**: Tell the user and suggest `/harneeslab-dev plan`
...
Save to `.claude/harneeslab/reports/{slug}-report.md` using the plan's slug.
...
```bash
mkdir -p .claude/harneeslab/plans/completed
mv {plan-path} .claude/harneeslab/plans/completed/
```
...
To continue: `/harneeslab-dev plan {prd-path}`
```

**Next steps**:
- To commit: `/harneeslab-dev commit`
- To create PR: `/harneeslab-dev pr`
- To review: `/harneeslab-dev review`
