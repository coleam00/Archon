...
Check `.claude/harneeslab/` for artifacts related to this work:

```bash
ls .claude/harneeslab/reports/ 2>/dev/null
ls .claude/harneeslab/plans/completed/ 2>/dev/null
ls .claude/harneeslab/issues/ 2>/dev/null
ls .claude/harneeslab/debug/ 2>/dev/null
```
...
Save to `.claude/harneeslab/reviews/{date}-pr-{number}.md` (or `{date}-{slug}.md` for local changes).
...
gh pr review {NUMBER} --approve --body-file .claude/harneeslab/reviews/{filename}.md

# If --request-changes OR high issues:
gh pr review {NUMBER} --request-changes --body-file .claude/harneeslab/reviews/{filename}.md

# Otherwise just comment:
gh pr comment {NUMBER} --body-file .claude/harneeslab/reviews/{filename}.md
```
...
Suggest: `/harneeslab-dev review {pr-number}` to self-review before requesting human review.
