...
If `--follow-up` and an existing research file on this topic exists in `.claude/harneeslab/research/`:
...
Save to `.claude/harneeslab/research/{date}-{slug}.md`.
...
**Next steps**:
- To dig deeper: `/harneeslab-dev research --follow-up {topic}`
- For strategic/external research: `/harneeslab-dev investigate {topic}`
- To write requirements: `/harneeslab-dev prd {topic}`
