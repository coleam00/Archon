...
Read this file thoroughly. Every section contains enforceable rules. Root
`CLAUDE.md` is only a one-line pointer to it — reading that instead gets you
nothing.
...
| **Natural Language Is Not a Wire Format** | Regexes, keyword lists, or hand-written parsers reconstructing user intent from prose; one fixed action applied to arbitrary prose ("any message here means approve"); a workflow node told to emit a sentinel token for a later node to grep, instead of `output_format` + `when:`, an exit code, or `until_bash`. NOT this rule: classifying third-party output (git stderr, SDK error strings), or repairing a model's own JSON that you then schema-validate |
