...
`output_format` (structured JSON output — all providers, schema-validated, node fails on miss), `hooks` (tool interception — Claude only), `mcp` (external tool servers — Claude + Codex), `skills` (domain knowledge injection — Claude/Pi per-node, Codex via filesystem), `allowed_tools`/`denied_tools` (tool restrictions — Claude + Pi), `persist_session` (cross-run AI memory), `output_type` (typed artifact sidecars).
