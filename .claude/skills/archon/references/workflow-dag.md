...
| Field | Type | Description |
|-------|------|-------------|
| `modelReasoningEffort` | `'minimal'` \| `'low'` \| `'medium'` \| `'high'` \| `'xhigh'` | **DEPRECATED** — use `effort:` above, which reaches Codex and works per-node. Still honoured (and still wins over `effort:` on Codex) but warns at load time |
