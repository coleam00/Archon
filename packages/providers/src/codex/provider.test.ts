...
describe('buildTurnOptions', () => {
  test('returns hasOutputFormat false when no output format specified', () => {
    const { turnOptions, hasOutputFormat } = buildTurnOptions(undefined);
    expect(hasOutputFormat).toBe(false);
    expect(turnOptions.outputSchema).toBeUndefined();
  });

  test('uses requestOptions.outputFormat schema', () => {
    const schema = { type: 'object', properties: {} };
    const { turnOptions, hasOutputFormat } = buildTurnOptions({
      outputFormat: { type: 'json_schema', schema },
    });
  ...
  test('uses nodeConfig.output_format when no requestOptions.outputFormat', () => {
  ...
  test('prefers requestOptions.outputFormat over nodeConfig', () => {
    const reqSchema = { type: 'object', properties: { a: { type: 'number' } } };
    const nodeSchema = { type: 'object', properties: { b: { type: 'string' } } };
    const { turnOptions } = buildTurnOptions({
      outputFormat: { type: 'json_schema', schema: reqSchema },
      nodeConfig: { output_format: nodeSchema },
    });
    expect(turnOptions.outputSchema).toEqual(reqSchema);
  });

  test('passes abort signal to turn options', () => {
    const controller = new AbortController();
    const { turnOptions } = buildTurnOptions({ abortSignal: controller.signal });
    expect(turnOptions.signal).toBe(controller.signal);
  });
});

describe('streamCodexEvents', () => {
  test('normalizes agent_message to assistant chunk', async () => {
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield {
        type: 'item.completed',
        item: { type: 'agent_message', text: 'Hi', id: '1' },
      };
      yield { type: 'turn.completed', usage: defaultUsage };
    }
    const chunks = [];
    for await (const chunk of streamCodexEvents(events(), false, 'tid')) {
      chunks.push(chunk);
    }
    expect(chunks[0]).toEqual({ type: 'assistant', content: 'Hi' });
    expect(chunks[1]).toMatchObject({ type: 'result', sessionId: 'tid' });
  });

  test('normalizes command_execution to tool + tool_result', async () => {
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield {
        type: 'item.completed',
        item: {
    ...
    expect(chunks[0]).toEqual({ type: 'tool', toolName: 'ls' });
    expect(chunks[1]).toEqual({ type: 'tool_result', toolName: 'ls', toolOutput: 'files\n' });
  });

  test('accumulates text and parses structured output on turn.completed', async () => {
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield {
        type: 'item.completed',
        item: { type: 'agent_message', text: '{"result": 42}', id: '3' },
      };
      yield { type: 'turn.completed', usage: defaultUsage };
    }
    const chunks = [];
    for await (const chunk of streamCodexEvents(events(), true, 'tid')) {
      chunks.push(chunk);
    }
    const resultChunk = chunks.find(c => c.type === 'result');
    expect(resultChunk).toBeDefined();
    expect(resultChunk!.type === 'result' && resultChunk!.structuredOutput).toEqual({ result: 42 });
  });

  test('emits warning when structured output is not valid JSON', async () => {
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield {
        type: 'item.completed',
        item: { type: 'agent_message', text: 'not json', id: '4' },
      };
      yield { type: 'turn.completed', usage: defaultUsage };
    }
    const chunks = [];
  ...
  test('deduplicates todo_list events with same signature', async () => {
    const todo = {
      type: 'todo_list',
    ...
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield { type: 'item.completed', item: todo };
      yield { type: 'item.completed', item: todo };
      yield { type: 'turn.completed', usage: defaultUsage };
    }
    const chunks = [];
  ...
  test('handles turn.failed event', async () => {
    ...
    expect(chunks[0]).toEqual({ type: 'system', content: '❌ Turn failed: boom' });
  });
  ...
  test('stops on abort signal', async () => {
    ...
    controller.abort();
    async function* events(): AsyncIterable<Record<string, unknown>> {
      yield { type: 'item.completed', item: { type: 'agent_message', text: 'hi', id: '6' } };
      yield { type: 'turn.completed', usage: defaultUsage };
    }
    const chunks = [];
    for await (const chunk of streamCodexEvents(events(), false, 'tid', controller.signal)) {
      chunks.push(chunk);
    }
...
describe('classifyAndEnrichCodexError', () => {
  test('classifies model access errors as non-retryable', () => {
    const result = classifyAndEnrichCodexError(new Error('model not available for your account'));
    expect(result.errorClass).toBe('model_access');
    expect(result.shouldRetry).toBe(false);
    expect(result.enrichedError.message).toContain('not available');
  });

  test('classifies auth errors as non-retryable', () => {
    const result = classifyAndEnrichCodexError(new Error('unauthorized'));
  ...
  test('classifies rate limit as retryable', () => {
    const result = classifyAndEnrichCodexError(new Error('rate limit reached'));
    expect(result.errorClass).toBe('rate_limit');
    expect(result.shouldRetry).toBe(true);
  });

  test('classifies crash as retryable', () => {
    const result = classifyAndEnrichCodexError(new Error('codex exec failed'));
    expect(result.errorClass).toBe('crash');
    expect(result.shouldRetry).toBe(true);
  });

  test('classifies unknown errors as non-retryable', () => {
    const result = classifyAndEnrichCodexError(new Error('something else'));
    expect(result.errorClass).toBe('unknown');
    expect(result.shouldRetry).toBe(false);
    expect(result.enrichedError.message).toContain('Codex unknown');
  });
});
