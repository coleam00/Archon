 ...
 *
 * Of the SendQueryOptions fields, `model`, `systemPrompt`, and
 * `abortSignal` are forwarded to the /api/chat payload. Provider-specific
 * options (model default, baseUrl) flow through `assistantConfig`.
 *
...
import { parseOllamaConfig } from './config';
 ...
 * Implements IAgentProvider via the Ollama REST API.
 */
export class OllamaProvider implements IAgentProvider {
   ...
   * Model is required — resolved from options.model, then assistantConfig.model.
   * baseUrl is resolved from assistantConfig.baseUrl, then OLLAMA_BASE_URL env, then default.
   */
  async *sendQuery(
    prompt: string,
    _cwd: string,
    ...
    const assistantCfg = parseOllamaConfig(options?.assistantConfig ?? {});
    const model = options?.model ?? assistantCfg.model;
    ...
    const baseUrl = assistantCfg.baseUrl ?? this.baseUrl;
  ...
  /** Returns Ollama's capability flags. */
