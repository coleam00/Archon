...
describe('CopilotSdkProvider', () => {
  ...
  test('nodeConfig.skills forwarded to sessionConfig.skillDirectories', async () => {
    await consume(
      new CopilotSdkProvider().sendQuery('hi', '/tmp', undefined, {
        nodeConfig: { skills: ['/home/user/.skills/coding', '/repo/.skills/custom'] },
      })
    );

    const [sessionConfig] = mockCreateSession.mock.calls[0] as [Record<string, unknown>];
    expect(sessionConfig.skillDirectories).toEqual([
      '/home/user/.skills/coding',
      '/repo/.skills/custom',
    ]);
  });

  test('empty nodeConfig.skills → skillDirectories absent from sessionConfig', async () => {
    await consume(
      new CopilotSdkProvider().sendQuery('hi', '/tmp', undefined, {
        nodeConfig: { skills: [] },
      })
  ...
  test('no nodeConfig.skills → skillDirectories absent from sessionConfig', async () => {
    await consume(new CopilotSdkProvider().sendQuery('hi', '/tmp'));
  ...
  test('streaming: true always set in sessionConfig', async () => {
    await consume(new CopilotSdkProvider().sendQuery('hi', '/tmp'));
