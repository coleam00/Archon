...
const validOmpConfigInput = {
  ...
  settings: {
    ...
    tools: { approvalMode: 'write', maxTimeout: 30 },
    providers: {
      webSearch: 'gemini',
      webSearchExclude: ['brave', 'searxng'],
      image: 'openrouter',
    },
    task: { maxConcurrency: 8, maxRuntimeMs: 60000 },
    ...
    expect(
      parseOmpConfig({
        ...
        settings: {
          ...
          tools: { approvalMode: 'prompt', maxTimeout: -1 },
          providers: {
            webSearch: 'unknown',
            webSearchExclude: ['brave', 'unknown', 3],
            image: 'stable-diffusion',
          },
          task: { maxConcurrency: -1, maxRuntimeMs: 1.5 },
    ...
    expect(
      parseOmpConfig({
        settings: {
          ...
          task: { maxConcurrency: 0, maxRuntimeMs: 0 },
          memory: { backend: 'off' },
        },
      })
    ).toEqual({
