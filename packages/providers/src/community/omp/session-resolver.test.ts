  ...
  return {
    SessionManager: {
      ...
      async forkFrom(filePath: string, cwd: string, sessionDir?: string) {
        return { kind: 'fork', filePath, cwd, sessionDir };
      },
    },
  };
}
...
describe('resolveOmpSession', () => {
  ...
  test('forks matching session when forkSession is requested', async () => {
    await expect(
      resolveOmpSession(
  ...
  test('falls back to fresh session when forkSession is requested but unsupported', async () => {
    ...
    await expect(resolveOmpSession(sdk, '/repo', 'abc', undefined, true)).resolves.toEqual({
      sessionManager: { kind: 'create', cwd: '/repo', sessionDir: undefined },
      resumeFailed: true,
    });
  });

  test('throws when forking a matching session fails unexpectedly', async () => {
    const sdk: Pick<OmpCodingAgentSdk, 'SessionManager'> = {
      SessionManager: {
        getDefaultSessionDir() {
          return '/tmp/omp-sessions';
    ...
    await expect(resolveOmpSession(sdk, '/repo', 'abc', undefined, true)).rejects.toThrow(
      "Oh My Pi session fork failed for 'abc': fork failed"
    );
  });
