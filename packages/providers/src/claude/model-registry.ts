...
let cachedModelRegistry: ClaudeModelRegistry | undefined;

/**
 * Return the cached custom Claude model registry, loading it on first use.
 */
export function getModelRegistry(): ClaudeModelRegistry {
  if (!cachedModelRegistry) {
    cachedModelRegistry = new ClaudeModelRegistry();
    cachedModelRegistry.load();
  }
  return cachedModelRegistry;
}

/**
 * Reload the cached custom Claude model registry from disk.
 */
export function reloadModelRegistry(): ClaudeModelRegistry {
  const registry = cachedModelRegistry ?? new ClaudeModelRegistry();
  registry.load();
...
/**
 * Clear the cached registry so the next getModelRegistry() call reloads from disk.
 */
