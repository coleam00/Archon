...
const { ClaudeModelRegistry, getModelRegistry, reloadModelRegistry, clearModelRegistryCache } =
...
function createLoadedRegistry(): InstanceType<typeof ClaudeModelRegistry> {
...
describe('ClaudeModelRegistry', () => {
  ...
  describe('file loading', () => {
    test('constructor does not load models until load is called', () => {
      writeModelsConfig(SAMPLE_CONFIG);
      const registry = new ClaudeModelRegistry();

      expect(registry.getAll()).toEqual([]);
      expect(registry.resolve('fast').matchedBy).toBe('passthrough');

      registry.load();
      expect(registry.resolve('fast').matchedBy).toBe('name');
    });

    test('getModelRegistry caches the loaded registry until explicitly reloaded', () => {
      writeModelsConfig(SAMPLE_CONFIG);
      const first = getModelRegistry();
      expect(first.resolve('fast').matchedBy).toBe('name');
      ...
      const second = getModelRegistry();
      expect(second).toBe(first);
      expect(second.resolve('other').matchedBy).toBe('passthrough');

      const reloaded = reloadModelRegistry();
      expect(reloaded).toBe(first);
      expect(reloaded.resolve('other').matchedBy).toBe('name');
    });
    ...
    test('reports error when providers field is null or an array', () => {
      ...
      const nullRegistry = createLoadedRegistry();
      ...
      const arrayRegistry = createLoadedRegistry();
