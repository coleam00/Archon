...
async function executeFanOutWorkflowNode(
  ...
  // 6. join: first_success (racing, PR-D) — the FIRST child to reach schema-valid terminal
  ...
  //     first_success node. Shares the items resolution, re-entry map, #2180 pre-checks,
  //     and the shared-checkout preflight above — racing children are concurrent siblings
  //     like any others, so they collide on the path lock the same way.
  ...
  // 7. Execute the incomplete indices through a bounded sliding window. Classification per
  ...
  // 8. #2180 (first-run path): a freshly-spawned child that paused at a gate fails the
