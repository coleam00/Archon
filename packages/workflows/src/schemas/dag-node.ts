...
/**
 * Workflow node schema — invokes a named child workflow as a node.
 * The child workflow runs synchronously; its terminal output becomes this node's $nodeId.output.
 * AI-specific base fields are present in the type but ignored at runtime.
 */
...
/** DAG node that invokes a named child workflow synchronously */
...
/** AI-specific fields that are meaningless on workflow nodes — same as bash nodes */
...
export const dagNodeSchema = dagNodeBaseSchema
  ...
  .superRefine((data, ctx) => {
    ...
    if (!hasBash && !hasLoop && !hasScript && !hasWorkflow && data.provider && data.model) {
      try {
...
/** Type guard: check if a DAG node is a workflow (child workflow invocation) node */
