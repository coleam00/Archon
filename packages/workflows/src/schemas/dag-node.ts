...
export const dagNodeSchema = dagNodeBaseSchema
  ...
  .superRefine((data, ctx) => {
    ...
    // but reject it fail-fast now. `$INPUTS` exists nowhere in the engine today, so an
    // author writing `as: task` and `$INPUTS.task` in the child gets the literal string
    // delivered to the model — silently wrong output with no error. A field that quietly
    // does nothing reads as a working feature.
