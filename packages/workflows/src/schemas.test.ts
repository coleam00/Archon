...
// ---------------------------------------------------------------------------
// dagNodeSchema — what a loop: node carries out of the transform
//
...
describe('dagNodeSchema — LoopNode AI-field survival', () => {
  ...
  /**
   * The one place the list and the transform still disagree at this commit:
   * LOOP_NODE_AI_FIELDS omits `model`/`provider` — claiming a loop node supports
   * them — while the loop branch drops both, so the override never reaches the
   * executor. That is the regression PR #3326 fixes, not this one, so it is
   * carved out here rather than silently skipped.
   *
   * This carve-out clears itself: once the transform preserves the two fields,
   * the case below expects them dropped and fails, and whoever lands that change
   * deletes these two names.
   */
  const DROPPED_DESPITE_CLAIMED_SUPPORT = new Set(['model', 'provider']);
  ...
  test('a field survives the transform exactly when LOOP_NODE_AI_FIELDS omits it', () => {
    ...
    for (const field of [...BASH_NODE_AI_FIELDS, 'output_format']) {
      ...
      const shouldSurvive =
        !LOOP_NODE_AI_FIELDS.includes(field) && !DROPPED_DESPITE_CLAIMED_SUPPORT.has(field);
