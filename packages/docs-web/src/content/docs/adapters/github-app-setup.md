...
> **Compiled binary builds:** the credential helper is installed from `scripts/git-credential-archon.sh` in the source tree. Compiled binaries that don't ship `scripts/` on disk silently skip the install. The initial clone still succeeds through request-scoped credentials, but later authenticated Git operations fail without the helper. Track this when running App mode in a binary deployment.
