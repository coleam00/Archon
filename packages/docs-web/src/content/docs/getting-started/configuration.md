...
| 변수 | 필수 | 설명 |
|----------|----------|-------------|
| `CLAUDE_BIN_PATH` | 예(binary builds) | Claude Code SDK의 `cli.js`에 대한 절대 경로입니다. `assistants.claude.claudeBinaryPath`가 설정되지 않은 compiled HarneesLab binary에서는 필수입니다. Dev mode(`bun run`)에서는 `node_modules`를 통해 자동으로 해석됩니다. |
