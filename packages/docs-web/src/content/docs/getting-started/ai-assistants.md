...
HarneesLab은 Archon fork이므로 assistant provider 이름, config key, CLI 명령은 upstream Archon과 동일하게 유지합니다.

## Claude Code

**Claude Pro/Max 구독자에게 권장합니다.**

HarneesLab은 Claude Code를 bundle하지 않습니다. 별도로 설치한 뒤, compiled HarneesLab binaries에서는 HarneesLab이 해당 실행 파일을 가리키도록 설정해야 합니다. dev(`bun run`)에서는 HarneesLab이 `node_modules`를 통해 자동으로 찾습니다.
...
compiled HarneesLab binaries는 runtime에 Claude Code를 자동 탐색할 수 없습니다. 다음 중 하나로 path를 제공하세요.
...
compiled HarneesLab binaries에서 `codex`가 Archon이 기대하는 default PATH에 없다면 다음 중 하나로 path를 제공하세요.
