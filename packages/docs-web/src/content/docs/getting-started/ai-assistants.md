---
title: AI 어시스턴트
description: HarneesLab에서 AI assistant로 Claude Code, Codex, Pi를 설정합니다.
...
HarneesLab은 Claude Code를 bundle하지 않습니다. Claude Code를 별도로 설치한 뒤, compiled HarneesLab binary에서는 HarneesLab이 해당 실행 파일을 가리키도록 설정해야 합니다. dev mode(`bun run`)에서는 `node_modules`를 통해 자동으로 찾습니다.
...
compiled HarneesLab binary는 runtime에 Claude Code를 자동 탐색할 수 없습니다. 다음 중 하나로 path를 제공하세요.
...
compiled binary에서 둘 다 설정되어 있지 않으면, 첫 Claude query 시 HarneesLab이 설치 안내와 함께 명확한 오류를 냅니다.
...
| Docker (`ghcr.io/newturn2017/harneeslab`) | image 안에서 `ENV CLAUDE_BIN_PATH`로 미리 설정됨 — 추가 작업 불필요 |
...
assistants:
  claude:
    ...
    # Required in compiled HarneesLab binaries if CLAUDE_BIN_PATH is not set.
    # claudeBinaryPath: /absolute/path/to/claude
```

`settingSources` option은 Claude Code SDK가 어떤 `CLAUDE.md` 파일을 load할지 제어합니다. 기본적으로 project-level `CLAUDE.md`만 load됩니다. 개인 `~/.claude/CLAUDE.md`도 함께 load하려면 `user`를 추가하세요.
...
HarneesLab은 Codex CLI를 bundle하지 않습니다. Codex CLI를 설치한 뒤 인증하세요.
...
compiled HarneesLab binary에서 `codex`가 HarneesLab이 확인하는 default PATH에 없다면 다음 중 하나로 path를 제공하세요.
...
Pi는 `@harneeslab/providers`의 dependency로 포함되어 있으므로 별도 설치가 필요하지 않습니다. 즉시 사용할 수 있습니다.

### 인증

Pi는 OAuth subscription과 API key를 모두 지원합니다. HarneesLab의 Pi adapter는 `pi` → `/login` 실행으로 생성되는 기존 Pi credentials(`~/.pi/agent/auth.json`)와 env vars를 모두 읽습니다. env vars가 request별로 우선하므로 codebase-scoped override가 가능합니다.
...
| 기능 | 지원 여부 | YAML field |
|---|---|---|
| Session resume | 지원 | automatic(HarneesLab이 `sessionId`를 persist) |
...
- 설정 우선순위는 workflow-level options > config file defaults > SDK defaults 순서입니다.
