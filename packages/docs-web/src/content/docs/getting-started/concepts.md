...
HarneesLab이 다루는 Archon fork의 동작 방식은 네 가지 핵심 개념으로 이해할 수 있습니다. 이 개념들을 먼저 잡아두면 나머지 문서와 워크플로 구조가 훨씬 자연스럽게 연결됩니다.
...
3. **실패한 run이 작업 공간을 어지럽히지 않습니다.** `hlab isolation cleanup`으로 정리할 수 있습니다.
