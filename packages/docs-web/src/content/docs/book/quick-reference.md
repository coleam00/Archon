...
| Command | 설명 |
|---------|-------------|
| `hlab workflow list` | 사용 가능한 모든 workflow 나열 |
| `hlab workflow list --json` | machine-readable JSON output |
| `hlab workflow run <name> "<prompt>"` | workflow 실행 |
| `hlab workflow run <name> --branch <name> "<prompt>"` | 명시적 브랜치로 실행 |
| `hlab workflow run <name> --no-worktree "<prompt>"` | live checkout에서 실행(isolation 없음) |
| `hlab workflow run <name> --cwd /path "<prompt>"` | 특정 디렉터리를 대상으로 실행 |
| `hlab workflow status` | 활성 workflow run 상태 표시 |
| `hlab workflow resume <run-id>` | 실패한 workflow run 재개 |
| `hlab workflow abandon <run-id>` | terminal 상태가 아닌 workflow run 포기 |
| `hlab workflow cleanup [days]` | 오래된 workflow run record 삭제(기본: 7일) |

### `hlab isolation`

| Command | 설명 |
|---------|-------------|
| `hlab isolation list` | 모든 활성 worktree 나열 |
| `hlab isolation cleanup` | 오래된 worktree 제거(7일 초과) |
| `hlab isolation cleanup <days>` | N일보다 오래된 worktree 제거 |
| `hlab isolation cleanup --merged` | main에 merge된 branch의 worktree 제거 |
| `hlab isolation cleanup --merged --include-closed` | 닫힌(abandoned) PR의 worktree도 제거 |

### `hlab complete`

| Command | 설명 |
|---------|-------------|
| `hlab complete <branch>` | worktree, local branch, remote branch 제거 |
| `hlab complete <branch> --force` | uncommitted-change check 건너뛰기 |

### `hlab validate`

| Command | 설명 |
|---------|-------------|
| `hlab validate workflows` | 모든 workflow definition 검증 |
| `hlab validate workflows <name>` | 단일 workflow 검증 |
| `hlab validate workflows <name> --json` | machine-readable validation output |
| `hlab validate commands` | 모든 command file 검증 |
| `hlab validate commands <name>` | 단일 command 검증 |
...
hlab workflow run my-workflow "auth"
# $ARGUMENTS = "auth", $1 = "auth"

# Multi-argument
hlab workflow run my-workflow "auth refresh-tokens"
...
| Field | 필수 | Type | 설명 |
|-------|----------|------|-------------|
| `name` | 예 | string | `hlab workflow list`에서 workflow 식별 |
...
hlab isolation list         # Any stale worktrees?
```
...
```bash
hlab workflow run my-workflow --no-worktree "..."
```
