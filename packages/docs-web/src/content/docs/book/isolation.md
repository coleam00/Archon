...
| 명령 패턴 | 동작 |
|-----------------|----------|
| `hlab workflow run <name> "..."` | 브랜치 이름을 자동 생성하고 격리된 worktree에서 실행 |
| `hlab workflow run <name> --branch my-branch "..."` | 지정한 브랜치 이름을 사용하고 격리된 worktree에서 실행 |
| `hlab workflow run <name> --no-worktree "..."` | isolation 없이 현재 디렉터리에서 직접 실행 |
...
```bash
hlab isolation cleanup 14   # Remove worktrees older than 14 days
```
...
```bash
hlab complete fix/issue-42
```

이것이 전체 lifecycle 종료입니다. merge 후 실행하면 깨끗한 상태로 돌아갑니다.

> **안전 참고**: Archon은 uncommitted change가 있는 worktree를 제거하지 않습니다. `cleanup`이 worktree를 건너뛰면 수동 삭제 전에 `hlab isolation list`로 확인하세요.
...
**merge 후 정리하세요.** PR이 반영되면 `hlab complete <branch>` 또는 `hlab isolation cleanup --merged`를 실행하세요. worktree 몇 개는 괜찮지만 오래된 것이 수십 개 쌓이면 혼란스러워집니다.
