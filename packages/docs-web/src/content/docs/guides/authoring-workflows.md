...
| `capture_tools` | string | — | Opt-in tool-result capture directory inside this run's artifacts, on an agent or loop node. Supports workflow/output references. Writes receipts atomically before completion; persistence failure fails the node. |
