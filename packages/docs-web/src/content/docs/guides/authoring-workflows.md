...
The recorded commit is always the one Git compared the worktree against; if HEAD keeps
moving while the checkout is read, the observation is `unavailable` rather than a guess.
