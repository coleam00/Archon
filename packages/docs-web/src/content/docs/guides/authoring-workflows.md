...
| `capture_tools` | string | — | Opt-in tool-result capture directory inside this run's artifacts, on an agent or loop node. Supports workflow/output references. Passes from reasks, loop iterations, and retries share one owned, bounded manifest. Writes receipts atomically before completion; persistence failure fails the node. The selected provider must expose capture payloads; unsupported or incomplete provider results remain unavailable and cannot qualify as evidence. |
