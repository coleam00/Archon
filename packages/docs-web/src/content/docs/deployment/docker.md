...
# GITHUB_TOKEN=ghp_...
# GH_TOKEN=ghp_...  # Optional gh-specific override
```
...
`gh auth login` inside the container can be used instead of env-based `gh` auth. By default Archon only needs `GITHUB_TOKEN`; set `GH_TOKEN` as well only if you intentionally want `gh` to prefer env auth:
