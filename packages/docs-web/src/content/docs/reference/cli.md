...
hlab workflow run assist --cwd /path/to/repo --no-worktree "Quick question"
```
...
```bash
hlab setup
hlab setup --spawn  # Open in a new terminal window
```
...
```bash
hlab workflow cleanup        # Default: 7 days
...
```bash
hlab validate workflows                 # Validate all workflows
...
hlab complete feature-auth --force  # bypass uncommitted-changes check
```
