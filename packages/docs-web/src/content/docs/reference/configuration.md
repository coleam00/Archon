...
A Better Auth session resolves to the **canonical** `remote_agent_users` row via the `web` platform identity, so chat/CLI/forge identities and the `role` column live on the one Archon user — Better Auth is only the login mechanism. Better Auth owns four tables prefixed `remote_agent_auth_*` (`user`/`session`/`account`/`verification`), applied automatically on startup. Every web request resolves a `{ userId, role }` auth context (session first, then the trusted header); `role` defaults to `admin`. On an install where identity arrives — web auth enabled, **or** `ARCHON_WEB_AUTH_HEADER` set — operator conversations (`web` and `cli`) are private to their owning user: `GET /api/conversations` returns only the caller's own, and a request whose identity does not resolve gets 401 rather than everyone's list. Conversations on chat and forge platforms keep today's behavior behind the `/api/*` gate. Conversations that predate enforcement have no owner and are therefore reachable by nobody; reattach them with [`archon conversations claim`](/reference/cli/#conversations). `GET /api/workflows/runs?mine=true` is still a non-enforcing convenience filter, not a security boundary: run reads stay open by design.
