...
| `first_success` racing join ([#1764](https://github.com/coleam00/Archon/issues/1764)) | ✅ shipped | A join rule — coordination. The engine sees only *which* outcome to keep; what makes a child worth keeping is either nothing (any success) or the node's already-declared `output_format`. No new expression surface, and the selection is not authored — it is temporal |
| Runtime sub-runs (`workflow:`, #2121 Phase 2) | ✅ shipped | A sub-run is a governance object (own run record, own gates, own audit trail). Slice 1: shared checkout, `input:` string, gate-aware pause/resume. Slice 2 adds opt-in per-child isolation (`isolation: worktree`) and data-driven fan-out (`fan_out:`) with all three joins including racing; `with:` remains deferred |
