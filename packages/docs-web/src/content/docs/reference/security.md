...
- **The gate proves an identity exists; ownership decides what that identity may reach.** On any install where identity actually arrives — web auth enabled, **or** `ARCHON_WEB_AUTH_HEADER` set — a `web` or `cli` conversation belongs to the user who created it. Opening it, reading its history, streaming it live over `/api/stream/<id>`, renaming it, deleting it, sending into it, and starting a workflow run in it all return `404` for anyone else, the same response as a conversation that does not exist (the ids are semi-guessable, so a `403` would confirm one exists). A conversation Archon cannot attribute — `user_id IS NULL`, which covers everything created before you turned auth on — is reachable by **nobody**: unknown is not public. To bring that pre-enforcement history back, claim it from the shell with [`archon conversations claim`](/reference/cli/#conversations) — never over HTTP, where the first user to click would take everyone else's history; it only ever moves rows that have no owner. Conversations on chat and forge platforms (Slack, Discord, Telegram, GitHub, GitLab, Gitea) are unchanged; those platforms own their own access model. The shared `/api/stream/__dashboard__` stream is unchanged as well — it is an install-wide operations view carrying run status, node transitions, and step counts, never chat content. Solo and SQLite installs are unchanged too: nothing is owned, so nothing is refused.
- **Workflow runs are shared to see and owned to act on.** Run *reads* stay open under enforcement: the runs list, run detail, artifacts, and the dashboard show every run on the install, because an operations team that cannot see what is running has lost the point of the dashboard. Run *actions* — cancel, resume, signal, abandon, approve, reject, respond, and delete — require the person who started the run and return `403` for anyone else. `403` rather than the conversations' `404`: a run id is an opaque UUID nobody guesses, and the caller can already see the run listed. Ownership is the run's own `user_id`, falling back to the owner of the conversation it was dispatched from; a run Archon cannot attribute is actionable by nobody over the web API. Note that run reads still expose `conversation_id` and `user_id` for conversations the viewer cannot open, and free-text subagent activity stays visible install-wide on the dashboard.
