...
description: Qualified PR lifecycle, work-item, comments, checks and forge plugins.
---
...
(or pass JSON directly as `--request`). The CLI selects the operation; credentials
...
write with a lost response reads the existing PR. An ambiguous head or different
existing content refuses. Body edits and ready are idempotent. GitHub ready uses
...
The implemented capabilities also include `pr.view`, `pr.create`, `pr.edit-body`,
`pr.ready`, `workitem.view` and `comment.upsert`. Resolve is the
...
Merge and trigger operations remain follow-ups. Per-user/per-host credential storage,
