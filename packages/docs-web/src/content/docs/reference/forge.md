...
in Archon's per-user GitHub mode, on workflow re-entry, and for explicitly scrubbed
tokens: service hosts must inject the selected user's credential. No provider configuration is replaced.
