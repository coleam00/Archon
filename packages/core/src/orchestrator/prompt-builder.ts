...
- \`archon workflow cancel <run-id> [--json]\` — stop a running run: a run another process owns has that process stopped first; a sub-run stops at its next status check; it refuses (pointing at abandon) when no owner answers
