...
export function foldPersistedDagNodes(events: WorkflowEventResponse[]): DagNodeState[] {
  ...
  for (const event of events.filter(candidate =>
    candidate.event_type.startsWith('loop_iteration_')
  )) {
    ...
    const iterIdx = existingIters.findIndex(item => item.iteration === iteration);
    ...
    queryFn: async (): Promise<WorkflowRunQueryData> => {
      ...
      return {
        workflowState: {
          ...
          dagNodes: foldPersistedDagNodes(data.events),
