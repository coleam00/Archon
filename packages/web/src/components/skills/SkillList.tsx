...
type SourceFilter = 'all' | 'global' | 'project' | 'symlinked';
...
export function SkillList({
  ...
  const counts = useMemo(() => {
    const c = { all: all.length, global: 0, project: 0, symlinked: 0 };
    for (const s of all) {
      c[s.source] += 1;
      if (s.isSymlink) c.symlinked += 1;
    }
    return c;
  }, [all]);
  ...
  const filtered = useMemo(() => {
    let list = all;
    if (filter === 'global' || filter === 'project') {
      list = list.filter(s => s.source === filter);
    } else if (filter === 'symlinked') {
      list = list.filter(s => s.isSymlink);
    }
    if (query.trim()) {
      ...
      list = list.filter(
        s =>
          ...
          (s.prefix?.toLowerCase().includes(q) ?? false)
      );
    }
    list = [...list];
    ...
    <div className="flex h-full w-[380px] shrink-0 flex-col border-r border-bridges-border-subtle bg-bridges-surface">
      ...
      <div className="flex flex-col gap-2.5 border-b border-bridges-border-subtle px-4 pt-3.5 pb-3">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[16px] font-semibold leading-tight text-bridges-fg1">Skills</div>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <FilterChip
            label="Symlinked"
            count={counts.symlinked}
            active={filter === 'symlinked'}
            onClick={() => {
              setFilter('symlinked');
            }}
          />
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && (
          <div className="px-6 py-10 text-center text-[13px] text-bridges-fg3">Loading skills…</div>
        )}
        {isError && (
          <div className="px-6 py-10 text-center text-[13px] text-bridges-tint-danger-fg">
            Failed to load skills: {(error as Error | undefined)?.message}
          </div>
        )}
        ...
        {filtered.map(s => (
          <SkillRow
            key={`${s.source}:${s.name}`}
            skill={s}
            selected={selectedName === s.name && selectedSource === s.source}
            onSelect={() => {
              onSelect(s);
            }}
          />
