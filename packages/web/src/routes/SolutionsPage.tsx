...
const STATUS_STYLE: Record<string, string> = {
  ...
  dormant: 'bg-zinc-800/40 text-zinc-400 border-zinc-700/40',
...
export function SolutionsPage(): React.ReactElement {
  ...
  const generatedAt =
    (solutionsData as { generated_at?: string }).generated_at ?? '';
  ...
  const selected: Solution | null =
    (selectedSlug && visibleSolutions.find((s) => s.slug === selectedSlug)) ||
    null;
    ...
    <div className="flex h-full flex-1 flex-col gap-4 p-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-text-secondary">
          Third-party solutions, distributorships, and strategic partners.
          Distinct from PMC sub-brands (which Jason owns) and clients (orgs
          Jason serves).
        </p>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-md border border-zinc-800 bg-zinc-900 py-2 pl-9 pr-3 text-sm text-text-primary placeholder:text-text-tertiary focus:border-zinc-600 focus:outline-none"
        />
      </div>
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        <div className="grid min-h-0 flex-1 auto-rows-max grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
          {filtered.length === 0 && (
            <div className="col-span-full rounded-lg border border-zinc-800 bg-zinc-950/40 p-8 text-center text-sm text-text-tertiary">
              No solutions match.
            </div>
          )}
          {filtered.map((s) => (
            <button
              key={s.slug}
              type="button"
              onClick={(): void => setSelectedSlug(s.slug)}
        ...
        {/* Detail pane (right) */}
        {selected && (
          <div className="w-96 shrink-0 overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-950/40 p-4">
            <div className="mb-3 flex items-start justify-between gap-2">
              ...
              <button
                type="button"
                onClick={(): void => setSelectedSlug(null)}
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
              ...
              {selected.model && (
                <span className="rounded-full border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-text-secondary">
                  {selected.model}
                </span>
              )}
              <span className="rounded-full border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-text-tertiary">
            ...
            {selected.tags && selected.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {selected.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded bg-zinc-900 px-1.5 py-0.5 text-[10px] text-text-tertiary"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="mt-4 border-t border-zinc-800 pt-3 text-[11px] text-text-tertiary">
              Edit the MOC in the vault at <code>{selected.vaultPath}</code>{' '}
              and this card refreshes within ~2s.
            </div>
          </div>
        )}
      </div>
      ...
      <p className="text-[11px] text-text-tertiary">
        Sourced from <code>second-brain/partners/</code>. PMC sub-brands (BRT,
        EWC, TTTS, IHHT, QEP, SG INK, bioreg.tech) live separately under{' '}
        <code>businesses/pmc/</code>. Client engagements (Cleveland Clinic,
        Precision Health) live under <code>clients-engagements/</code>.{' '}
