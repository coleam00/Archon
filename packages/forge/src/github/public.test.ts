...
describe('public GitHub operations through the owning boundary', () => {
  ...
  it('executes the public CLI and plugin subprocess protocol with audited, secret-free output', async () => {
    const f = fixture();
    ...
    try {
      const processResult = Bun.spawn(
        ...
        {
          stdin: new Blob([JSON.stringify({ ref, expected })]),
          stdout: 'pipe',
          stderr: 'pipe',
          env: { ...process.env, GH_TOKEN: 'fixture-secret', GITHUB_TOKEN: '', ARCHON_HOME: home },
        }
      );
