...
describe('workflowRunCommand', () => {
  ...
  it('executes a prepared pending run from its frozen source without recapturing', async () => {
    const run = {
      id: '11111111-1111-4111-8111-111111111111',
      workflow_name: 'assist',
      conversation_id: 'conversation-db-id',
      codebase_id: 'cb-prepared',
      user_id: 'user-prepared',
      user_message: 'queued input',
      status: 'pending' as const,
      working_path: '/test/path',
      started_at: new Date(),
      metadata: {
        inputs_values: { count: 2, enabled: true },
        workflow_source: {
          version: 1,
          root: '/test/capture',
          origin: '/source/at-intake',
    ...
    const frozenWorkflow = makeTestComposedWorkflow(
      [makeTestWorkflow({ name: 'assist' })],
      'assist'
    );
    const workflowDbModule = await import('@archon/core/db/workflows');
    (workflowDbModule.getWorkflowRun as ReturnType<typeof mock>).mockResolvedValueOnce(run);
    ...
    const prepare = executor.prepareWorkflowSource as ReturnType<typeof mock>;
    const captureCount = prepare.mock.calls.length;

    await workflowRunCommand('/test/path', 'assist', 'queued input', {
      noWorktree: true,
      preparedLaunch: {
        version: 1,
        run: {
        ...
        execution: {
          ...
          actingUserId: 'user-prepared',
          inputs: { count: 2, enabled: true },
          isolation: { kind: 'in-place' },
        },
      },
    });

    expect(mockResolveContinuationWorkflow).toHaveBeenCalledWith(
      expect.anything(),
      run,
      '/test/path'
    );
    expect(prepare.mock.calls.length).toBe(captureCount);
  });
