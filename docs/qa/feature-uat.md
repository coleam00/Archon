...
- [N] **At least one codebase is registered.** Click "Codebases" in the sidebar (or visit `http://localhost:5173/codebases`). Pass when: the list has at least one entry with a green status dot. Fail when: empty list — register one via "+ Add codebase" before proceeding; some sections below require a live codebase. [I do not think /codebases is a valid API route]
