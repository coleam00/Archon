...
#### 🚢 Local Docker Instance (Optional)

#####  Docker Networking Scope
set `SUPABASE_URL` to http://host.docker.internal:8000 (unless you have an IP address set up).
##### Generate new `SERVICE_ROLE_KEY`
1. Go to [jwt.io](https://jwt.io/)
2. Click `JWT Decoder` tab
3. Paste Supabase `JWT_SECRET` into `JWT Signature Verification` and ensure it is valid. Otherwise generate a new one using `Generate example` button with option `HS256`.
4. Click `JWT Encoder` tab
5. If not present, create the JSON header and paste it into `Header: Algorithm & Token Type`:
    ```json
    {
    "alg": "HS256",
    "typ": "JWT"
    }
6. Create the JSON payload for our service role and paste it into `Payload: Data`:
    ```json
    {
    "role": "service_role",
    "iss": "supabase",
...
7. Paste your valid `$JWT_SECRET` into the `Sign JWT: Secret` field.
8.  Copy the signed `JSON Web Token`. This will be your `SERVICE_ROLE_KEY` in Supabase .env and your `SUPABASE_SERVICE_KEY` Archon .env.

##### Generate new `ANON_KEY`
  1.  Repeat the same steps as the `SUPABASE_SERVICE_KEY` but at Step f. change the JSON payload to the following:
      ```json
      {
      "role": "anon",
      "iss": "supabase",
