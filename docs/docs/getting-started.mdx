...
#### ⚙️ Task Indexes (Performance)
<Admonition type="tip" icon="⚡" title="Task indexes for faster task lists">
- New installations: `migration/complete_setup.sql` already creates the task indexes for `archon_tasks` (composite on `(project_id, status, task_order)`). No extra action is required.
- Step-based setup: If you follow steps 1–3 above, run `migration/07_add_archon_tasks_indexes.sql` after Step 2 to add the same indexes.
- Upgrading existing installations: run `migration/07_add_archon_tasks_indexes.sql` in the Supabase SQL Editor. Execute statements individually because `CREATE INDEX CONCURRENTLY` cannot run inside a transaction.
...
-- Check query plan for typical list pattern
EXPLAIN ANALYZE
SELECT id
FROM archon_tasks
