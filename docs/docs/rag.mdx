...
When configuring custom OpenAI base URLs, follow these security best practices. These security measures apply to **all OpenAI API usage** throughout the system:

<Admonition type="danger" title="Security Requirements">
**Always provide an API key** when using custom base URLs. This prevents accidental traffic from being sent to unintended endpoints when a proxy is explicitly configured.
</Admonition>

### Network Security
```bash
# ✅ Use HTTPS endpoints in production
OPENAI_BASE_URL=https://secure-proxy.example.com/v1

# ⚠️ HTTP only for local development  
OPENAI_BASE_URL=http://localhost:8000/v1  # Development only
```
...
### Proxy Validation
Before deploying with a custom proxy:
1. **Verify SSL/TLS certificates** for HTTPS endpoints
2. **Test connectivity** and authentication
3. **Validate response formats** match OpenAI API standards
4. **Monitor for unexpected behavior** or response changes

### Corporate Environments
```bash
# Corporate proxy with additional security headers
OPENAI_BASE_URL=https://api-gateway.corp.com/openai/v1
